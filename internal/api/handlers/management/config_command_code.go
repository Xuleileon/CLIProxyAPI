...
func (h *Handler) GetCommandCodeKeys(c *gin.Context) {
	c.JSON(200, gin.H{"command-code-api-key": h.commandCodeKeysWithAuthIndex()})
}
...
func (h *Handler) PutCommandCodeKeys(c *gin.Context) {
	...
	var entries []config.CommandCodeKey
...
func (h *Handler) PatchCommandCodeKey(c *gin.Context) {
	...
	if index == -1 && body.Match != nil {
		for i := range h.cfg.CommandCodeKey {
			if h.cfg.CommandCodeKey[i].APIKey == strings.TrimSpace(*body.Match) {
				index = i
				break
			}
		}
	...
	entry := h.cfg.CommandCodeKey[index]
	...
	h.cfg.CommandCodeKey[index] = entry
	h.cfg.SanitizeCommandCodeKeys()
	h.persistLocked(c)
}
...
func (h *Handler) DeleteCommandCodeKey(c *gin.Context) {
	...
	if rawIndex := c.Query("index"); rawIndex != "" {
		var index int
		if _, errScan := fmt.Sscanf(rawIndex, "%d", &index); errScan == nil && index >= 0 && index < len(h.cfg.CommandCodeKey) {
			h.cfg.CommandCodeKey = append(h.cfg.CommandCodeKey[:index], h.cfg.CommandCodeKey[index+1:]...)
			h.persistLocked(c)
			return
		}
	}
