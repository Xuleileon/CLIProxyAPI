...
func EnsureGeminiTrailingUserContent(payload []byte, path string) []byte {
	...
	if (lastRole != "model" && lastRole != "assistant") || translatorcommon.ContentHasGeminiFunctionResponse([]byte(lastContent.Raw)) {
		return payload
	}
...
// EnsureAntigravityGeminiBoundaryUserContent leaves Claude targets unchanged;
// their adapter rejects empty text parts.
func EnsureAntigravityGeminiBoundaryUserContent(model string, payload []byte) []byte {
	if strings.Contains(strings.ToLower(model), "claude") {
		return payload
	}
	return EnsureGeminiBoundaryUserContent(payload, "request.contents")
}
