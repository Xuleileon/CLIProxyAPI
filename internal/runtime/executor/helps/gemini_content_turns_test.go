...
func TestAntigravityBoundaryPreservesClaudeAndFunctionResponses(t *testing.T) {
	for _, tc := range []struct{ model, payload string }{
		{"claude-sonnet-4-5", `{"request":{"contents":[{"role":"model","parts":[{"text":"answer"}]}]}}`},
		{"gemini-2.5-pro", `{"request":{"contents":[{"role":"user","parts":[{"functionResponse":{"name":"read","response":{"result":"ok"}}}]}]}}`},
	} {
		if got := EnsureAntigravityGeminiBoundaryUserContent(tc.model, []byte(tc.payload)); string(got) != tc.payload {
			t.Fatalf("%s payload changed: %s", tc.model, got)
		}
	}
}
