...
func (s *GitTokenStore) ensureRepositoryLocked() error {
	...
	if _, err := os.Stat(gitDir); errors.Is(err, fs.ErrNotExist) {
		...
		if errVerify := verifyRepositoryHead(repo); errVerify != nil {
			...
			if errRecover := s.recoverRepositoryLocked(repoDir, authMethod, nil, nil, repo); errRecover != nil {
				s.dirLock.Unlock()
		...
		if errPull := worktree.Pull(pullOpts); errPull != nil {
			...
			case errors.Is(errPull, git.NoErrAlreadyUpToDate):
				if errReset := resetIndexToHead(repo, worktree); errReset != nil {
					...
					if errRecover := s.recoverRepositoryLocked(repoDir, authMethod, prePullTree, dirtyPaths, repo); errRecover != nil {
						s.dirLock.Unlock()
...
func (s *GitTokenStore) recoverRepositoryLocked(repoDir string, authMethod []client.Option, baselineTree *object.Tree, dirtyPaths map[string]struct{}, activeRepo *git.Repository) (errRecovery error) {
	repositoriesToClose := []*git.Repository{activeRepo}
	defer func() {
		if errClose := closeGitRepositories(repositoriesToClose); errClose != nil {
	...
	if baselineTree == nil {
		inspectedTree, inspectedDirtyPaths, inspectedRepo, errInspect := inspectRecoveryBaseline(repoDir)
		if errInspect != nil {
			return fmt.Errorf("inspect recovery baseline: %w", errInspect)
		}
		repositoriesToClose = append(repositoriesToClose, inspectedRepo)
	...
	repositoriesToClose = append(repositoriesToClose, clonedRepo)
	...
	if errClose := closeGitRepositories(repositoriesToClose); errClose != nil {
		return fmt.Errorf("close repositories before recovery install: %w", errClose)
	}
	repositoriesToClose = nil
	...
	if errClose := closeGitRepositories([]*git.Repository{recoveredRepo}); errClose != nil {
		errOpen = errors.Join(errOpen, fmt.Errorf("close recovered repository: %w", errClose))
	}
	if errOpen != nil {
...
func inspectRecoveryBaseline(repoDir string) (*object.Tree, map[string]struct{}, *git.Repository, error) {
	repo, errOpen := git.PlainOpen(repoDir)
	if errOpen != nil {
	...
	worktree, errWorktree := repo.Worktree()
	if errWorktree != nil {
		_ = closeGitRepositories([]*git.Repository{repo})
	...
	return tree, dirtyPaths, repo, nil
}

func closeGitRepositories(repositories []*git.Repository) error {
	var errClose error
	seen := make(map[*git.Repository]struct{}, len(repositories))
