...
func TestBuildAPIKeyClientsCounts(t *testing.T) {
	...
	gemini, vertex, claude, codex, xai, openCodeGo, compat := BuildAPIKeyClients(cfg)
	if gemini != 3 || vertex != 1 || claude != 1 || codex != 2 || xai != 1 || openCodeGo != 1 || compat != 2 {
		t.Fatalf("unexpected counts: %d %d %d %d %d %d %d", gemini, vertex, claude, codex, xai, openCodeGo, compat)
	}
}
