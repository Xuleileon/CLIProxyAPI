...
func isBootstrapAuthSelectionUnavailable(err error) bool {
...
func enrichBootstrapAuthSelectionError(err error, providers []string, model string) error {
	...
	return &coreauth.Error{
		...
		Message:    fmt.Sprintf("no auth available during stream bootstrap retry (providers=%s, model=%s)", providerText, modelText),
		Retryable:  true,
...
func isModelCooldownError(err error) bool {
	if err == nil || statusFromError(err) != http.StatusTooManyRequests {
		return false
	}
	var payload struct {
		Error struct {
	...
	return json.Unmarshal([]byte(err.Error()), &payload) == nil && payload.Error.Code == "model_cooldown"
}
