...
// disabled transient cooldowns, bypassing the exponential backoff ladder.
...
// for a transient failure. Concurrent failures inside one cooldown window reuse
// the same deadline so the ladder advances at most once per window.
