...
func isEmptyCompletion(chunks []cliproxyexecutor.StreamChunk) bool {
	...
	for _, c := range chunks {
		if detector.state.observeChunk(c.Payload) {
			return false
		}
	}
...
// observeChunk accepts the executor contract: complete SSE event/data lines may
// arrive separately without newlines. Keep Observe's raw byte-stream semantics
// intact for callers that split opaque SSE metadata at arbitrary byte offsets.
func (s *streamBootstrapState) observeChunk(fragment []byte) bool {
	pending := bytes.TrimSpace(s.pending)
	if bytes.HasPrefix(pending, []byte("event:")) && bytes.HasPrefix(bytes.TrimSpace(fragment), []byte("data:")) {
		event := strings.TrimSpace(string(pending[len("event:"):]))
		if strings.HasPrefix(event, "response.") && !strings.ContainsAny(event, " \t\r\n:") {
			s.processLine(pending)
			s.pending = s.pending[:0]
		}
	}
	return s.observe(fragment)
}
