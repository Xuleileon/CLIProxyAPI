...
func MergeRefreshedAuth(base, current, updated *Auth) *Auth {
	...
	hasNewConcurrentError := current.LastError != nil && (base == nil || !reflect.DeepEqual(current.LastError, base.LastError))
	...
	if finalDisabled {
		...
		if hasNewConcurrentError {
			...
			merged.Status = current.Status
			merged.StatusMessage = current.StatusMessage
		} else if current.Unavailable && current.NextRetryAfter.After(time.Now()) && !isUnauthorizedError(current.LastError) {
			...
			if isUnauthorizedError(current.LastError) {
				merged.NextRetryAfter = time.Time{}
			}
		}
	}
	...
	if updated.Metadata != nil {
		for k, v := range updated.Metadata {
			...
			if changedByExecutor {
				// Preserve newer credentials as well as concurrent user configuration.
				if !changedByUser {
					merged.Metadata[k] = v
				}
			}
